...
describe("Delete Team Integration Test;", () => {
    it("Delete Team; Valid Data; Success;", async () => {
        ...
        const response = await supertest(server).delete(`/api/teams/${team_001.id}/delete`).send(request).set("Content-Type", "application/json");

        expect(response.status).toBe(200);

        const [row] = await db.query<ITeamSchema | null>({ statement: `SELECT * FROM team WHERE id = ${team_001.id}` });
    ...
    it("Delete Team; Team does not exist; Failure;", async () => {
        ...
        const response = await supertest(server).delete(`/api/teams/100/delete`).send(request).set("Content-Type", "application/json");
